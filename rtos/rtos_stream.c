...
void app_main() {
   ...
   xStream = xStreamBufferCreate(STREAM_BUFFER_SIZE, 1);
...
static void vTask1( void *pvParameters ){
   ...
   for(;;){ 
      ...
      f_led5 ^= 1;
      xRecBytes = xStreamBufferReceive(xStream, ucRecBuffer, DATA_SIZE, portMAX_DELAY);
   }
}

static void vTask2( void *pvParameters ){

   uint8_t ucDataBuffer[DATA_SIZE];
   size_t xBytesSent;
   BaseType_t f_led4=0;
   ...
   for(;;){ 
      xBytesSent = xStreamBufferSend(xStream, ucDataBuffer, DATA_SIZE, portMAX_DELAY);
